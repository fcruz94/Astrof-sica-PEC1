...
class StarInputForm:
    ...
    def setup_ui(self):
        ...
        # Campos del formulario (ordenados lógicamente)
        ...
        # Nuevo campo: φ (desplazamiento angular)
        ...
        # Actualizar Δt cuando cambian las fechas
    ...
    def update_delta_time(self, event=None):
        """Calcula y muestra Δt cuando se seleccionan fechas"""
        try:
            date1 = self.date1_entry.get_date()
            date2 = self.date2_entry.get_date()
            if date1 and date2:
                delta = (date2 - date1).days / 365.25
                self.delta_time_label.config(text=f"{delta:.2f}")
        except:
            self.delta_time_label.config(text="0.00")
    ...
    def add_star(self):
        ...
        try:
            star_data = {
                ...
                'angular_displacement': float(self.angular_displacement_entry.get()),  # φ
                'date1': self.date1_entry.get_date(),
                'date2': self.date2_entry.get_date(),
                'delta_time': float(self.delta_time_label.cget("text")),
                'parallax': float(self.parallax_entry.get()),  # π
            ...
            # Validaciones
            if star_data['date1'] >= star_data['date2']:
                raise ValueError("La fecha de la 2ª observación debe ser posterior a la 1ª")
            if star_data['parallax'] <= 0:
                raise ValueError("El paralaje debe ser positivo")
                
            self.stars_data.append(star_data)
            self.clear_form()
            tk.messagebox.showinfo("Éxito", "Estrella añadida correctamente")
            
        except ValueError as e:
            tk.messagebox.showerror("Error", f"Datos inválidos: {str(e)}")
    
    def submit_data(self):
        """Envía todos los datos para procesamiento"""
        if not self.stars_data:
            tk.messagebox.showwarning("Advertencia", "No hay datos de estrellas para procesar")
            return
    ...
    def clear_form(self):
        ...
        self.date1_entry.set_date(None)
        self.date2_entry.set_date(None)
        self.delta_time_label.config(text="0.00")